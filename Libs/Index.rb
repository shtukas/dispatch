...
class Index
    ...
    # Index::location_is_dispatch_item(location)
    def self.location_is_dispatch_item(location)
        location[-19, 19] == ".dispatch-Nx06.json"
    end
    ...
    def self.uuid_to_directory(uuid, should_create_if_missing)
        ...
        directory = "#{Config::pathToDataRepository()}/index-Nx06/#{uuidhash[0, 2]}/#{uuidhash}"
    ...
    def self.sendItemToDisk(item)
        ...
        filename = "#{filehash}.dispatch-Nx06.json"
        ...
        filepaths_existing = LucilleCore::locationsAtFolder(directory)
                                .select{|location| Index::location_is_dispatch_item(location) }
    ...
    def self.getItems()
        items = []
        root = "#{Config::pathToDataRepository()}/index-Nx06"
        Find.find(root) do |path|
            if Index::location_is_dispatch_item(path) then
    ...
    #                    .select{|filepath| filepath[-19, 19] == ".dispatch-Nx06.json" }
    ...
    #    root = "#{Config::pathToDataRepository()}/index-Nx06"
    #    Find.find(root) do |path|
    #        if path[-19, 19] == ".dispatch-Nx06.json" then
    ...
    # Index::accessItemDirectory(uuid)
    def self.accessItemDirectory(uuid)
        uuidhash = Digest::SHA1.hexdigest(uuid)
        directory = "#{Config::pathToDataRepository()}/index-Nx06/#{uuidhash[0, 2]}/#{uuidhash}"
